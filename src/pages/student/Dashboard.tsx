...
import { BookOpen, Video, Calendar, Bell, ArrowUpRight } from 'lucide-react';
...
export default function StudentDashboard() {
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        const [coursesRes, lessonsRes, eventsRes, announcementsRes, progressRes] = await Promise.all([
        ...
        if (coursesRes.ok && lessonsRes.ok && eventsRes.ok && announcementsRes.ok) {
  ...
  const statCards = [
    { name: 'My Courses', value: stats.courses, icon: BookOpen, path: '/student/courses' },
    { name: 'Video Lessons', value: stats.lessons, icon: Video, path: '/student/lessons' },
    { name: 'Upcoming Events', value: stats.events, icon: Calendar, path: '/student/events' },
    { name: 'Announcements', value: stats.announcements, icon: Bell, path: '/student/announcements' },
  ];
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-px bg-zinc-200 border border-zinc-200">
          ...
          <div className="md:col-span-2 bg-white p-10 relative overflow-hidden group">
            ...
            <div className="relative z-10 flex flex-col h-full justify-between min-h-[300px]">
              <div>
                <p className="text-xs font-semibold text-zinc-400 uppercase tracking-widest mb-4">Course Progress</p>
              ...
              <div className="mt-12 flex flex-col gap-4">
                <div className="flex items-end justify-between">
                  <span className="text-8xl font-light tracking-tighter text-zinc-900">{progress.progress}%</span>
                  <Link to="/student/lessons" className="text-sm font-medium text-zinc-900 hover:text-zinc-500 uppercase tracking-wider flex items-center gap-2 transition-colors">
